...
# update.sh's last build step is `pm2 restart overlay`, which kills every
# open project terminal (pty child processes die with the server, see
...
# forces re-authentication (deploy/update.sh's last step, Authelia), so an
