...
Codepfad. `deploy/update.sh` (Schritt 6/7) installiert und aktiviert die
Unit seit dem Fix für "wiederkehrende Checks laufen nie automatisch"
(Abschnitt 16.2 hatte dasselbe Problem) automatisch bei jedem Update, falls
`/etc/systemd/system/overlay-check-update.timer` noch fehlt. Mit
